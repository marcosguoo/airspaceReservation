...
def place_random_bid(private_key):
    ...
    try:
        ...
        return  # Skip this bid and continue with the next one

    # Calculate the full gas fee and store the estimated fee
    full_gas_fee = estimated_gas * transaction['gasPrice']
    estimated_fees.append(full_gas_fee)

    # Update the transaction with the actual estimated gas
...
#time.sleep(5)
# Now it's safer to call end_auction
end_auction()

# After all bids have been placed and actual fees calculated
with open('transaction_fees.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    # Include new headers for Bid Amount and Bid Placed By
